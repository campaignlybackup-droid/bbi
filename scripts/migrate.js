...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS custom_ranking_pages (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    title TEXT NOT NULL,
    slug TEXT UNIQUE NOT NULL,
    description TEXT,
  ...
  CREATE TABLE IF NOT EXISTS custom_ranking_items (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (page_id) REFERENCES custom_ranking_pages(id),
    FOREIGN KEY (business_id) REFERENCES businesses(id),
    UNIQUE(page_id, business_id)
  );
  ...
  CREATE TABLE IF NOT EXISTS digest_entries (
    ...
    city_id INTEGER,
    category_id INTEGER,
    movement_value INTEGER DEFAULT 0,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
  );
  ...
  CREATE INDEX IF NOT EXISTS idx_faqs_page ON faqs(page_type, entity_id);
  CREATE INDEX IF NOT EXISTS idx_digest_month ON digest_entries(month);
  CREATE INDEX IF NOT EXISTS idx_search_clicks_query ON search_clicks(query);
  CREATE INDEX IF NOT EXISTS idx_places_place_id ON google_places_cache(place_id);
`);
...
safeAddColumn('ranking_scores', 'strict_rank', 'INTEGER DEFAULT NULL');

// Businesses - Google Places + email
safeAddColumn('businesses', 'place_id', 'TEXT');
safeAddColumn('businesses', 'email', 'TEXT');

// Businesses - Custom attributes (JSON)
safeAddColumn('businesses', 'custom_attributes', 'TEXT');

// Businesses - Import system fields
safeAddColumn('businesses', 'services', 'TEXT');
safeAddColumn('businesses', 'facebook_url', 'TEXT');
safeAddColumn('businesses', 'instagram_url', 'TEXT');
safeAddColumn('businesses', 'linkedin_url', 'TEXT');
safeAddColumn('businesses', 'import_id', 'INTEGER');
safeAddColumn('businesses', 'bbi_score', 'REAL DEFAULT 0');
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS csv_import_rows (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (import_id) REFERENCES csv_imports(id),
    FOREIGN KEY (business_id) REFERENCES businesses(id)
  );
  ...
  CREATE INDEX IF NOT EXISTS idx_csv_import_rows_import ON csv_import_rows(import_id);
  CREATE INDEX IF NOT EXISTS idx_csv_import_rows_business ON csv_import_rows(business_id);
  CREATE INDEX IF NOT EXISTS idx_audit_logs_entity ON audit_logs(entity_type, entity_id);
  CREATE INDEX IF NOT EXISTS idx_audit_logs_created ON audit_logs(created_at);
  CREATE INDEX IF NOT EXISTS idx_businesses_import ON businesses(import_id);
`);
...
console.log('   csv_imports, csv_import_rows, audit_logs');
console.log('   FTS5 virtual table: businesses_fts');
console.log('   Indexes: 25+ indexes created');
db.close();
