  ...
  try {
    ...
    switch (job.job_type) {
      ...
      case 'combo_seo_generate':
        ...
        break;
      case 'generate_state_category_content':
      case 'generate_area_category_content':
      case 'generate_usecase_content':
      case 'generate_variation_content':
        if (typeof provider.generateProgrammaticSeoContent === 'function') {
           result = await provider.generateProgrammaticSeoContent(job.job_type, job.payload);
        } else {
           // Fallback / Stub
           const seoService = require('../seoService');
           const entityType = job.job_type.replace('generate_', '').replace('_content', '');
           seoService.saveSeoContent(entityType, job.payload.id, {
              title: `Programmatic Title for ${entityType} ID ${job.payload.id}`,
              meta_description: `Programmatic description for ${entityType} ID ${job.payload.id}`,
              editorial_content: `Content dynamically generated by AI worker for job type ${job.job_type}`,
              ai_summary: '',
              faq_json: '[]'
           });
           result = { success: true };
