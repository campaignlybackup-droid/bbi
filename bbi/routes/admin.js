...
router.post('/businesses', requireAuth, (req, res) => {
  const { name, category_id, city_id, address, phone, website, description, google_rating, google_review_count, verified, sponsored, tags } = req.body;
  const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
  try {
    const bizId = db.prepare(`
      INSERT INTO businesses (name,slug,category_id,city_id,address,phone,website,description,google_rating,google_review_count,verified,sponsored,tags)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
    `).run(name, slug, category_id, city_id, address, phone, website, description,
      parseFloat(google_rating)||0, parseInt(google_review_count)||0, verified?1:0, sponsored?1:0, tags||''
    ).lastInsertRowid;
...
router.post('/businesses/:id', requireAuth, (req, res) => {
  const { name, category_id, city_id, address, phone, website, description, google_rating, google_review_count, verified, sponsored, editorial_score, manual_boost, tags } = req.body;
  db.prepare(`
    UPDATE businesses SET name=?,category_id=?,city_id=?,address=?,phone=?,website=?,description=?,
    google_rating=?,google_review_count=?,verified=?,sponsored=?,tags=?,updated_at=CURRENT_TIMESTAMP
    WHERE id=?
  `).run(name, category_id, city_id, address, phone, website, description,
    parseFloat(google_rating)||0, parseInt(google_review_count)||0, verified?1:0, sponsored?1:0, tags||'', req.params.id);
...
router.get('/seo', requireAuth, (req, res) => {
  ...
  res.render('admin/seo-list', { cities, categories, title: 'SEO Pages', admin: req.session, flash: { success: req.flash('success') } });
});
...
router.get('/seo/:type/:id', requireAuth, (req, res) => {
  ...
  if (!entity) return res.redirect('/admin/seo');
  ...
  res.render('admin/seo-form', { type, entity, seoContent, title: \`Edit SEO: \${entity.name}\`, admin: req.session, flash: { error: req.flash('error') } });
});
...
router.post('/api/ai/generate', requireAuth, async (req, res) => {
  try {
    ...
    if (type === 'listing') {
      ...
      result = await aiProvider.generateSeoTags(businessInfo.name, businessInfo.type);
    } else if (type === 'seo_editorial') {
      result = await aiProvider.generateSeoEditorial(businessInfo.name, businessInfo.type);
