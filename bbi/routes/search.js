...
router.get('/search', (req, res) => {
  ...
  const searchResult = searchService.search(q, { city_id, category_id });
  const trending = searchService.getTrending(8);
  const recentSearches = searchService.getRecentSearches(5);

  // Log search
  if (q.length > 0) {
    searchService.logSearch(q, searchResult.total, req.ip);
  }
  ...
  // Destructure search results — rename to avoid shadowing the dropdown variables
  const { results, categories: matchedCategories, cities: matchedCities, total, query } = searchResult;
...
router.post('/api/search/click', express.json(), (req, res) => {
  const { query, resultId, resultType } = req.body;
  if (query && resultId) {
    searchService.logClick(query, resultId, resultType || 'business');
  }
